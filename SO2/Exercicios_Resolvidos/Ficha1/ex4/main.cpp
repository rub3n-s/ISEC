...
using namespace std; //Permitir que o mesmo código possa funcionar para ASCII ou UNICODE 
#ifdef UNICODE 
	#define tcout wcout 
	#define tcin wcin 
...
//A maneira mais fácil para ter esta funcionalidade é chamar _setmode:
	...
	tcout << str; 
	do{ 
		getline(tcin, str); //Maiúsculas 
		for (unsigned int i = 0; i < str.length(); i++)
			str[i] = _totupper(str[i]);
		tcout << TEXT("Frase: ") << str << TEXT("\nTamanho:") << str.length() << endl;
	} while (str != TEXT("FIM"));
