...
const ControlPanel = ({ gameStarted, onGameStart, selectedLevel, onLevelChange }) => {
  useEffect(() => {
  ...
  const toggleClass = (elements) => {
    Array.from(elements).forEach((element) => {
      element.classList.contains("gameStarted")
    ...
    <section id="panel-control">
      ...
      <form className="form">
        <fieldset className="form-group">
          ...
          <select id="btLevel" disabled={gameStarted} onChange={onLevelChange}>
            ...
            <option value="1">Básico (2x3)</option> <option value="2">Intermédio (3x4)</option>
            <option value="3">Avançado (4x5)</option>
          </select>
        </fieldset>
        <button type="button" id="btPlay" disabled={gameStarted} onClick={onGameStart}>
          Iniciar Jogo
        </button>
      </form>
      <div className="form-metadata">
        ...
        <dl className="list-item left">
          <dt>Tempo de Jogo:</dt> <dd id="gameTime">0</dd>
        </dl>
