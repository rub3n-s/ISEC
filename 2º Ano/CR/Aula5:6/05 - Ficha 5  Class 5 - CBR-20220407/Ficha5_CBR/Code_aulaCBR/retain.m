function [case_library] = retain(case_library, new_case, journey_index)
    ...
    % verificar se o atributo esta em falta, e caso esteja preencher pelo
    % valor do vizinho mais proximo
    ...
    if ~isfield(new_case,'number_persons')
        new_case.number_persons = case_library{journey_index,4};
    end

    if ~isfield(new_case,'reason')
        new_case.reason = case_library{journey_index,5};
    end
