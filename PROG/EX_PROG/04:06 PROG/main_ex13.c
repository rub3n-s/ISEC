...
int Melhor_Cliente_joao(pCliente p)
{
    int HremMinIN, HremMinOUT, entrada, saida, tempo, maiorTempo=0, idMaior;
    pAcesso aux;
    ...
    while(p!=NULL)
    {
        ...
        while(aux!=NULL)
        {
            ...
            HremMinIN = (aux->in.h)*60;
            HremMinOUT = (aux->out.h)*60;
            entrada = HremMinIN + aux->in.m;
            saida = HremMinOUT + aux->out.m;
            tempo +=(saida - entrada);
            aux = aux->prox;
            }
         }
...
pCliente acessoParque(pCliente p, int id, hora x)
{
    ...
    auxC =p;
    while(auxC->prox != NULL && auxC->id != id)
        auxC = auxC->prox;

    if(auxC->prox == NULL && auxC->id != id)
    {
        novoC = malloc(sizeof(cliente));
        if(novoC == NULL)
        {
...
int main(){
    ...
    printf("\n");
    printf("\nO cliente que esteve mais tempo no parque tem o ID: %d", Melhor_Cliente_joao(lista));
    printf("\n");



    lista = acessoParque(lista, 10, x);
    escreve_tudo(lista);
    printf("\n");
